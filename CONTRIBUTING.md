...
1. Open [src/utils/constants.js](src/utils/constants.js) and add your persona details to the `PERSONAS` configuration.
2. Update [src/services/personas.js](src/services/personas.js) to define the fallback generation rules and unique tones.
3. Update [src/components/PersonaReviews/PersonaReviews.jsx](src/components/PersonaReviews/PersonaReviews.jsx) if any custom colors/icons are needed.

### 2. Easter Egg
To add a new easter egg (e.g. keypress combinations, secret commands, mouse hover triggers):
1. Implement your physics or state checks in [src/services/easterEggs.js](src/services/easterEggs.js).
2. Wire up the state transitions in the [AppContext.jsx](src/context/AppContext.jsx) reducer.
3. Add a visual component in [src/components/EasterEggs/EasterEggs.jsx](src/components/EasterEggs/EasterEggs.jsx) wrapped in `AnimatePresence` and `motion` transitions.
